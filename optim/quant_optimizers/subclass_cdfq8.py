...
class OptimStateCDFQ8(TorchAOBaseTensor):
    ...
    def _fake_quantize(self):
        _, _, tmp = cdf_quantization(self.codes, 8, self.block_size)
...
@OptimStateCDFQ8.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, OptimStateCDFQ8) and isinstance(src, OptimStateCDFQ8):
        assert dst.block_size == src.block_size
        dst.codes.copy_(src.codes)
        dst.scale.copy_(src.scale)

    elif isinstance(dst, OptimStateCDFQ8):
...
@OptimStateCDFQ8.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
...
@OptimStateCDFQ8.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, OptimStateCDFQ8) else x for x in args]
    return func(*args, **kwargs)

# Needed for the quant error compute
@OptimStateCDFQ8.implements(aten.sub.Tensor)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, OptimStateCDFQ8) else x for x in args]
    return func(*args, **kwargs)

# this is needed for DTensor.from_local()
@OptimStateCDFQ8.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args
    return OptimStateCDFQ8(x.codes.view(shape), x.scale)


@OptimStateCDFQ8.implements(
    [
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, OptimStateCDFQ8):
        raise ValueError(f"expecting a OptimStateCDFQ8 but found {type(x)}")

    # assume tensors from all ranks have the same signedness
    return OptimStateCDFQ8(
...
@OptimStateCDFQ8.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
...
@OptimStateCDFQ8.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
...
if TORCH_VERSION_AT_LEAST_2_5:
    ...
    add_safe_globals([OptimStateCDFQ8])