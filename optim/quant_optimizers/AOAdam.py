"""
    Attempt to circumvent AssertionError: fp8e4nv data type is not supported on CUDA arch < 89
    Simulate torchao floating point types. -> No need! It was only a matter of not using torch.compile
    
    Idea:
        Copy the BaseAdam.py folder here.
        Change the subclass_fp8.py  to not instantiate fp8, but to just simulate it.
...
from .subclass_cdfq8 import OptimStateCDFQ8
from .subclass_cdfq4 import OptimStateCDFQ4
from .subclass_cdfq3 import OptimStateCDFQ3
from .subclass_cdfq2 import OptimStateCDFQ2
from .subclass_gq4 import OptimStateGQ4
from .subclass_gq3 import OptimStateGQ3

OptimState8bit = lambda x: False
_fp32_to_bf16_sr = lambda x: False
            ...
            for group in self.param_groups:
                for p in group["params"]:
                    ...
                    if len(state) == 0:
                        ...
                        state["q_error_exp_avg"] = 0
                        state["q_error_exp_avg_sq"] = 0
                        if group["amsgrad"]:
                    ...
                    # torch.compile(single_param_adam, fullgraph=True, dynamic=False, backend='inductor')(
                    #     p.detach(),
                    #     grad,
                    #     state["step"],
                    #     state["exp_avg"],
                    ...
                    q_error_exp_avg, q_error_exp_avg_sq = single_param_adam(
                        p.detach(),
                        grad,
                        state["step"],
                        state["exp_avg"],
                    ...
                    state["q_error_exp_avg"]   += q_error_exp_avg
                    state["q_error_exp_avg_sq"]+= q_error_exp_avg_sq 
        return loss
...
def single_param_adam(
    ...
    if BF16_STOCHASTIC_ROUND:
        ...
        p.copy_(p_f32)
    q_error_exp_avg = (exp_avg_f32-exp_avg.float()).abs().sum()
    q_error_exp_avg_sq = (exp_avg_sq_f32-exp_avg_sq.float()).abs().sum()
    return q_error_exp_avg, q_error_exp_avg_sq
...
class AdamCDFQ8(_AdamBase):
    ...
    @staticmethod
    def _subclass_zeros(p: Tensor, signed: bool, block_size: int):
        return OptimStateCDFQ8.zeros(p.shape, block_size, p.device)


class AdamCDFQ4(_AdamBase):
    def __init__(
        self,
        params,
        lr=1e-3,
    ...
    @staticmethod
    def _subclass_zeros(p: Tensor, signed: bool, block_size: int):
        return OptimStateCDFQ4.zeros(p.shape, block_size, p.device)

class AdamCDFQ3(_AdamBase):
    def __init__(
        self,
        params,
        lr=1e-3,
    ...
    @staticmethod
    def _subclass_zeros(p: Tensor, signed: bool, block_size: int):
        return OptimStateCDFQ3.zeros(p.shape, block_size, p.device)


class AdamCDFQ2(_AdamBase):
    def __init__(
        self,
        params,
        lr=1e-3,
    ...
    @staticmethod
    def _subclass_zeros(p: Tensor, signed: bool, block_size: int):
        return OptimStateCDFQ2.zeros(p.shape, block_size, p.device)
  
class AdamGQ3(_AdamBase):
    def __init__(
        self,
        params,
        lr=1e-3,
    ...
    @staticmethod
    def _subclass_zeros(p: Tensor, signed: bool, block_size: int):
        return OptimStateGQ3.zeros(p.shape, block_size, p.device)

class AdamGQ4(_AdamBase):
    def __init__(
        self,
        params,
        lr=1e-3,
    ...
    @staticmethod
    def _subclass_zeros(p: Tensor, signed: bool, block_size: int):
        return OptimStateGQ4.zeros(p.shape, block_size, p.device)
