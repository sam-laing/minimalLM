...
class OptimStateGQ3(TorchAOBaseTensor):
...
@OptimStateGQ3.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, OptimStateGQ3) and isinstance(src, OptimStateGQ3):
        assert dst.block_size == src.block_size
        dst.codes.copy_(src.codes)
        dst.scale.copy_(src.scale)

    elif isinstance(dst, OptimStateGQ3):
...
@OptimStateGQ3.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
...
@OptimStateGQ3.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, OptimStateGQ3) else x for x in args]
    return func(*args, **kwargs)

# Needed for the quant error compute
@OptimStateGQ3.implements(aten.sub.Tensor)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, OptimStateGQ3) else x for x in args]
    return func(*args, **kwargs)

# this is needed for DTensor.from_local()
@OptimStateGQ3.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args
    return OptimStateGQ3(x.codes.view(shape), x.scale)

@OptimStateGQ3.implements(
    [
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, OptimStateGQ3):
        raise ValueError(f"expecting a OptimStateGQ3 but found {type(x)}")

    # assume tensors from all ranks have the same signedness
    return OptimStateGQ3(
...
@OptimStateGQ3.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
...
@OptimStateGQ3.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
...
if TORCH_VERSION_AT_LEAST_2_5:
    ...
    add_safe_globals([OptimStateGQ3])