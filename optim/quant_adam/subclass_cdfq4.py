...
class OptimStateCDFQ4(TorchAOBaseTensor):
...
@OptimStateCDFQ4.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, OptimStateCDFQ4) and isinstance(src, OptimStateCDFQ4):
        assert dst.block_size == src.block_size
        dst.codes.copy_(src.codes)
        dst.scale.copy_(src.scale)

    elif isinstance(dst, OptimStateCDFQ4):
...
@OptimStateCDFQ4.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
...
@OptimStateCDFQ4.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, OptimStateCDFQ4) else x for x in args]
    return func(*args, **kwargs)

# Needed for the quant error compute
@OptimStateCDFQ4.implements(aten.sub.Tensor)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, OptimStateCDFQ4) else x for x in args]
    return func(*args, **kwargs)

# this is needed for DTensor.from_local()
@OptimStateCDFQ4.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args
    return OptimStateCDFQ4(x.codes.view(shape), x.scale)

@OptimStateCDFQ4.implements(
    [
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, OptimStateCDFQ4):
        raise ValueError(f"expecting a OptimStateCDFQ4 but found {type(x)}")

    # assume tensors from all ranks have the same signedness
    return OptimStateCDFQ4(
...
@OptimStateCDFQ4.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
...
@OptimStateCDFQ4.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
...
if TORCH_VERSION_AT_LEAST_2_5:
    ...
    add_safe_globals([OptimStateCDFQ4])